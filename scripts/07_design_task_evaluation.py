...
Approach:
  1. Train models on Grasso training data (156 physicochemical features)
  2. Load design variants with features from the same xlsx
  3. Predict WA for each design variant
  4. Evaluate ranking accuracy per gene and overall

Features: Physicochemical (156d) from xlsx — available for ALL design variants.
(ESM2 embeddings exist only for train/test sequences, not design variants.)

Models evaluated:
  - RF with best ESM2-650M hyperparameters applied to PhysChem features (from Script 02)
...
from src.data_loading import (
    load_physicochemical, GRASSO_FEATURES,
...
def main():
    ...
    print(f"  Train: {meta['n_train']} samples, {meta['n_features']} features")
    ...
    # Extract features for design variants
    available_features = [f for f in GRASSO_FEATURES if f in df_design.columns]
    X_design = df_design[available_features].fillna(0).values.astype(np.float64)
    y_design = df_design['WA'].values.astype(np.float64)
    print(f"  Design feature matrix: {X_design.shape}")
    ...
    # Predict on design variants
    df_design['pred_rf'] = rf.predict(X_design_scaled)

    # ── 7. Train and predict: NN ensemble ──────────────────────────────────
    print(f"\n{'='*60}")
    ...
    for i, seed in enumerate(SEEDS):
        ...
        pred = nn.predict(X_design_scaled)
    ...
    # ── 8. Evaluate per-gene and overall ──────────────────────────────────
    print(f"\n{'='*60}")
    print("  Design Task Evaluation Results")
    print(f"{'='*60}")

    model_configs = [('RF', 'pred_rf'), ('NN', 'pred_nn')]

    for model_name, pred_col in model_configs:
        gene_results, overall = evaluate_per_gene(df_design, pred_col, wt_wa_map)
    ...
    print("  SUMMARY")
    print(f"{'='*60}")
    print(f"  {'Model':<10} {'Spearman':>10} {'Pearson':>10} {'MSE':>10} {'ClassAcc':>10} {'N':>6}")
    print(f"  {'-'*10} {'-'*10} {'-'*10} {'-'*10} {'-'*10} {'-'*6}")
    for model_name in ['RF', 'NN']:
        o = results['models'][model_name]['overall']
        ca = f"{o['mean_classification_accuracy']:.4f}" if o['mean_classification_accuracy'] else "N/A"
        print(f"  {model_name:<10} {o['spearman_rho']:>+10.4f} {o['pearson_r']:>+10.4f} "
              f"{o['mse']:>10.4f} {ca:>10} {o['n_variants']:>6}")

    # ── 10. Figure ─────────────────────────────────────────────────────────
    _make_figure(df_design, results, wt_wa_map)
...
def _make_figure(df_design, results, wt_wa_map):
    """Generate multi-panel design task evaluation figure."""
    fig, axes = plt.subplots(1, 3, figsize=(18, 5))

    model_colors = {'RF': 'steelblue', 'NN': 'darkorange'}
    model_labels = {'RF': 'RF', 'NN': 'NN Regression'}
    pred_cols = {'RF': 'pred_rf', 'NN': 'pred_nn'}

    # ── Panel A: Predicted vs Actual WA scatter ────────────────────────────
    ax = axes[0]
    for model_name, pred_col in pred_cols.items():
        ax.scatter(df_design['WA'], df_design[pred_col],
                   alpha=0.2, s=10, color=model_colors[model_name],
                   label=model_labels[model_name])

    # Diagonal line
    wa_min = min(df_design['WA'].min(), 1)
    wa_max = max(df_design['WA'].max(), 10)
    ax.plot([wa_min, wa_max], [wa_min, wa_max], 'k--', lw=0.8, alpha=0.5)
    ax.set_xlabel('Actual WA')
    ax.set_ylabel('Predicted WA')
    ax.set_title('(A) Predicted vs Actual WA (Design Variants)')
    ax.legend(fontsize=8)

    # ── Panel B: Per-gene Spearman bar chart (top 15 by n_variants) ────────
    ax = axes[1]
    all_genes = sorted(set.intersection(*[
        set(results['models'][m]['per_gene'].keys()) for m in model_colors
    ]))
    # Sort by n_variants descending, take top 15
    genes_by_n = sorted(all_genes,
                        key=lambda g: -results['models']['RF']['per_gene'][g]['n_variants'])
    genes = genes_by_n[:15]
    ...
    if genes:
        ...
        for i, (model_name, color) in enumerate(model_colors.items()):
            gene_rhos = [results['models'][model_name]['per_gene'][g]['spearman_rho']
                         for g in genes]
            ax.bar(x + i * width, gene_rhos, width,
                   label=model_labels[model_name], color=color, alpha=0.85)

        ax.set_xticks(x + width / 2)
        ax.set_xticklabels(genes, rotation=45, ha='right', fontsize=7)
        ax.set_ylabel('Spearman Rank Correlation')
        ax.set_title('(B) Per-Gene Ranking Accuracy (top 15)')
        ax.legend(fontsize=8)
        ax.axhline(y=0, color='gray', linewidth=0.8, linestyle='--')

    # ── Panel C: Per-gene classification accuracy (genes with WT) ──────────
    ax = axes[2]
    genes_with_wt = [g for g in genes
                     if results['models']['RF']['per_gene'][g]['classification_accuracy'] is not None]
    ...
    if genes_with_wt:
        ...
        for i, (model_name, color) in enumerate(model_colors.items()):
            accs = [results['models'][model_name]['per_gene'][g]['classification_accuracy']
        ...
        ax.set_title('(C) Better/Worse than WT Classification')
        ax.legend(fontsize=8)
    ...
    fig.suptitle('Design Task Evaluation: Predicting Mutation Effects (PhysChem Features)',
                 fontsize=13)
    plt.tight_layout()
