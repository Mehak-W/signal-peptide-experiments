...
def main():
    ...
    # ══════════════════════════════════════════════════════════════════════
    # Phase 1: Reproduce Schrier (single model, Ginkgo-AA0)
    # ══════════════════════════════════════════════════════════════════════
    print(f"\n{'='*60}")
    print("  PHASE 1: Reproduce Schrier's Result (single models)")
    print(f"{'='*60}")
    ...
    phase1_configs = {
        '200ep':         {**schrier_base, 'epochs': 200},
        '300ep':         {**schrier_base, 'epochs': 300},
        '500ep':         {**schrier_base, 'epochs': 500},
        '200ep-noLRred': {**schrier_base, 'epochs': 200, 'lr_reduce': False},
        '300ep-lr1e-3':  {**schrier_base, 'epochs': 300, 'lr': 1e-3},
        '300ep-cosine':  {**schrier_base, 'epochs': 300, 'lr_schedule': 'cosine'},
    }
    ...
    for name, cfg in phase1_configs.items():
        ...
        beat = " *** BEAT ***" if mse < SCHRIER_MSE else ""
    ...
    # ══════════════════════════════════════════════════════════════════════
    # Phase 2: Multi-seed ensemble (full data, Schrier config)
    # ══════════════════════════════════════════════════════════════════════
    print(f"\n{'='*60}")
    print("  PHASE 2: Full-Data Ensembles (Schrier config)")
    print(f"{'='*60}")

    ens_cfg = {**schrier_base, 'epochs': best_p1_epochs}
    ...
    beat = " *** BEAT ***" if mse_5 < SCHRIER_MSE else ""
    ...
    beat = " *** BEAT ***" if mse_10 < SCHRIER_MSE else ""
    ...
    for cfg_name in top3_names:
        ...
        beat = " *** BEAT ***" if mse_ens < SCHRIER_MSE else ""
    ...
    beat = " *** BEAT ***" if mse_mixed < SCHRIER_MSE else ""
    ...
    beat = " *** BEAT ***" if mse_ens_10 < SCHRIER_MSE else ""
    ...
    # Schrier config on ESM2-650M
    esm_cfg = {**schrier_base, 'epochs': best_p1_epochs}
    ...
    # Check if ESM beats overall best
    for label, preds_list, y_test_ref in [
        ('P5-ESM-5seed-Schrier', preds_esm_5, y_test_e_full),
    ...
    if overall_best_mse < SCHRIER_MSE:
        gap = SCHRIER_MSE - overall_best_mse
        pct = 100 * gap / SCHRIER_MSE
