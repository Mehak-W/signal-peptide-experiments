...
def main():
    ...
    print(f"  Reference: Schrier benchmark = {SCHRIER_MSE}")
    ...
    gap_schrier = best_esm - SCHRIER_MSE
    pct_schrier = 100 * gap_schrier / SCHRIER_MSE

    print()
    if best_esm < SCHRIER_MSE:
        print(f"  ESM2-650M best ({best_key}): {best_esm:.4f}")
        print(f"    -> BEATS Schrier ({SCHRIER_MSE}) by {-gap_schrier:.4f} ({-pct_schrier:.1f}%)")
    else:
        print(f"  ESM2-650M best ({best_key}): {best_esm:.4f}")
        print(f"    -> Gap to Schrier: +{gap_schrier:.4f} ({pct_schrier:+.1f}%)")
    ...
    all_results['summary'] = {
        ...
        'schrier_mse': SCHRIER_MSE,
        'gap_to_ginkgo': gap_ginkgo,
        'gap_to_ginkgo_pct': pct_ginkgo,
        'gap_to_schrier': gap_schrier,
        'gap_to_schrier_pct': pct_schrier,
        'cis_overlap': overlap,
